-- Generated CloudBase migration from database/shared/migrations/20260922160000_idea_plan_dates.sql.

-- Edit the shared source and the minimal provider overlay, then rebuild.

BEGIN;

-- Preserve saved Idea dates in the Plan item details as well as its selected Plan day.
...
BEGIN
  ...
  LOOP
    ...
    IF chosen_day_id IS NULL AND (source.category = 'activity' OR source.start_date IS NOT NULL) THEN
