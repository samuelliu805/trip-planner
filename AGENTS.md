...
- The root layout pins the viewport scale (`minimum-scale=1, maximum-scale=1`). Focusing a field on iPadOS is a zoom-to-fit rather than a scroll to reveal, which is why the page moved even when the field was already visible; with the scale unable to change there is nothing to fit. Do not add `user-scalable=no` on top — it costs every user WCAG 1.4.4 for a key iOS already ignores for a user's own pinch.
