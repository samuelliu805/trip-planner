...
- Renaming happens in a single field docked above the keyboard (`DockedFieldEditor`), reached from the trip menu — not in a form. A tall surface puts the field somewhere in the middle of the screen, which is exactly what the iPadOS keyboard moves the page to reveal; a field that is never covered gives it nothing to reveal. The dock lifts itself clear on `pointerdown`, before focus, because that is the last moment before the browser decides. Overshoot the lift rather than fall short, and never focus the field programmatically: iPadOS refuses that outside a gesture, so the traveller's own tap has to be what opens the keyboard.
