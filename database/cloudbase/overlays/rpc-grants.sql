-- CloudBase RPC grants mirror the current non-storage application RPC surface.
-- The CloudBase gateway currently does not enforce these ACLs, so SECURITY DEFINER functions
-- must still authenticate and authorize the caller internally.
DO $$
DECLARE
