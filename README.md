...
- Phase 6B-D/P0 — selection, Plan Cost, currency-safe comparison, and Plan-specific OptionImpact — DONE. Selections are persisted independently per route variant and decision slot; Cost now sums the priced canonical items in the active Plan exactly once and displays them in the Trip currency with dated reference-rate provenance.
- Phase 6B-E/P0 — Review, transactional Apply, durable history, and conflict-safe Revert — DONE for Flights, Stays, Trains, and Rentals. One confirmation selects and applies atomically, reconciles Plan dates/Days, and creates or updates canonical itinerary items.
- Phase 6B P0.5 — Trip App Bar and Context Bar convergence — DONE. Desktop keeps destination navigation in the App Bar; mobile keeps its contextual controls at the top and uses one iOS-style bottom destination bar for Plan and Ideas & Options.

The Trip-level user architecture is now **Plan | Ideas & Options**, implemented with real App Router navigation. “Research” remains an internal domain name, while the user-facing label describes the workspace more clearly. Provider integrations, live inventory/FX, extraction, and attachments remain intentionally deferred.
...
## Phase 6B Price Comparison / Research

Trip detail navigation uses real URLs: Plan at `/trips/[tripId]` and Ideas & Options at `/trips/[tripId]/compare/flights`, `/stays`, `/trains`, or `/rentals`. Category changes update the URL and filter the already-loaded owner data without another server render; browser Back/Forward remains meaningful. Plan-to-Ideas navigation prefetches the dynamic destination, while the Ideas route reads only the compact Plan snapshot needed for comparison. The Trip App Bar owns trip identity, Plan selection, desktop destination navigation, save state, Share, trip overflow, and the compact account menu. The Context Bar stays immediately below it and owns only the actions relevant to the current Plan selection or Ideas & Options category. On phones, Plan and Ideas & Options move to a stable safe-area-aware bottom tab bar while category/filter/action controls remain at the top. The former standalone section navigation and always-visible editing strip are gone; Compare no longer repeats the trip title.

Private owner-only `research_items` are categorized as Flight, Stay, Train, or Rental. A name, source URL, or note is enough to save; price and comparison context remain optional. A round-trip Flight asks for one From/To pair and infers the reverse return leg; multi-city retains explicit segments. Flight and Train locations use the existing Google Places autocomplete with airport/station/locality filters, while Stays and Rentals retain normalized Google Places. Date ranges move directly from departure/check-in/pickup to the matching return/check-out field. Readiness is derived from the same row, so completing the required price and category context turns an Idea into “Ready to compare” without creating an Option record.
...
Each route variant may select at most one ready ResearchItem for a stable decision slot. Slot identity prefers a canonical itinerary item, then Day/category context, then a normalized provider-neutral category/context fingerprint. Editing an option so it no longer belongs to that slot invalidates the selection instead of retaining a misleading relationship. Selecting another candidate atomically replaces only that variant's previous choice for the slot; other variants and saved alternatives remain unchanged.

**Cost** is a partial total derived from persisted canonical Plan prices, not every saved or selected Research alternative. Activity, Meal, Hotel, supported Transport/Flight/Train, and Rental pickup rows may carry one price/currency pair; Rental return and non-cost support rows cannot. Flight/Train and Rental bookings retain one intentional price anchor. Stay totals are allocated across their nightly Hotel rows in exact cents, so a two-night 500.00 Stay contributes 250.00 per night without changing the booking total. Reapplying cannot double-count a booking.

Plan and Decision Summary display Cost in the Trip currency and keep the itemized disclosure on the Plan side. Cross-currency display uses the dated European Central Bank daily reference table, converts through its EUR base, rounds each displayed contribution to two decimal places, and preserves every canonical item price in its captured currency. When a required reference rate is unavailable, the UI says so instead of showing a partial or fabricated total. Research price sorting remains currency-partitioned—Trip currency first, numeric only within a currency—so it never compares raw USD and JPY amounts. This reference-rate display needs no API key or paid provider and is not booking/settlement financial truth.

OptionImpact is derived against the inspected variant, not stored on ResearchItem. Exact fit, same-duration date shift, structural change, and manual/conflict review states are explicit. The normal path is one direct **Apply to Plan** action; a chooser appears only when multiple matching canonical items make the target genuinely ambiguous. A same-duration Flight rebases the Plan while preserving Day and Activity IDs; a longer range appends the missing Days; a shorter range removes only blank tail Days and otherwise preserves the user's planned tail. A captured price of zero is still a valid ready-to-apply reference, and choosing a Stay check-in defaults checkout to the following day while remaining editable.

Apply is the only Research-to-canonical mutation boundary. The owner-authorized `apply_research_item_to_variant_v2` entry point atomically selects the choice, reconciles required Plan Days, creates or updates canonical Transport/Hotel/Car-rental rows, distributes a Stay total across its nightly Hotel rows, and writes one narrow durable change set without calling Routes. Flight and Train segments become timed Transport rows; a Stay writes Hotel rows for each night; a Rental writes the canonical pickup/return pair. Repeating the same unchanged Apply is idempotent, while editing its Research source or affected canonical Plan data makes the old application non-current and permits a fresh Apply without duplicate items or cost. Revert compares current owned fields, Plan Days, and Primary summary fields to the recorded after snapshot. Clean matches restore prior values and remove untouched Apply-created rows/Days; later overlapping edits or dependencies return a reviewable conflict, while unrelated edits are preserved. Research and selection survive Apply/Revert. Primary Plan Days are schedule truth; `trips.start_date`, `end_date`, and `day_count` are a validated synchronized summary, with duration derived from the dated range in settings. Variant comparison and Decision Summary display Cost from canonical Plan price truth, while the explicit public projection remains unchanged.
...
Forward-only migration `20260810154805_phase_6b_plan_selection_apply_revert.sql` adds `variant_research_selections`, `research_plan_applications`, stable decision-slot/readiness helpers, selection invalidation, and atomic select/clear/apply/revert RPCs. Both tables are private owner workspace data with owner-only SELECT policies, explicit authenticated RPC execution, no anonymous access, composite trip-safe foreign keys, and no direct client mutation grants. Generated database types match the linked schema.

Forward-only migrations `20260810170830_apply_global_flight_to_unambiguous_plan.sql` and `20260811041528_apply_structural_days_and_rentals.sql` extend that same transaction boundary without adding a second planning model. Global Flights resolve only against an unambiguous active Plan; compatible longer Flight/Stay/Rental ranges append blank end Days; Rental Apply writes the canonical pickup/return pair; and structural Revert removes only untouched Apply-created Days and records. The public entry points retain owner-only execution while their internal compatibility helpers are revoked from browser roles.

Forward-only migration `20260811080457_research_apply_v2_schedule_and_details.sql` adds journey segments/times and normalized research Places, the idempotent active-application invariant, Primary Plan schedule-summary constraints, automatic date/Day reconciliation, canonical Flight/Train/Stay/Rental operations, supersession, and conflict-safe Revert for the expanded change set. `20260811084649_harden_research_journey_readiness.sql` makes incomplete round-trip/multi-city journeys ineligible at the authoritative RPC boundary while retaining legacy row compatibility. `20260811093500_secure_schedule_triggers_and_research_indexes.sql` revokes direct API execution from internal schedule-trigger functions and covers every new composite Research foreign key. Browser roles remain limited to the narrow owner-authorized product wrappers.

Forward-only migration `20260811121345_detach_deleted_research_history.sql` lets an owner delete Research without deleting canonical Plan data or durable Apply history by detaching the source reference transactionally. `20260811151042_canonical_plan_prices_and_current_applications.sql` adds constrained canonical Plan price/currency fields, price-aware Apply/Revert history, and the owner-only current-application projection. `20260811153435_harden_canonical_plan_price_eligibility.sql` enforces complete price/currency pairs and rejects cost on Rental return/support rows. `20260811154500_normalize_legacy_flight_journeys_before_apply.sql` normalizes legacy round trips to one outbound and one inferred reverse segment at the atomic Apply boundary.

`20260811170608_apply_complete_booking_fields.sql` makes the current Apply boundary replace the complete Research-owned booking field set, including clearing absent optional values, and records that full field set for conflict-safe Revert. It also repairs legacy duplicate Research price anchors and enforces one canonical price anchor per Research booking and Plan.

`20260811181150_distribute_stay_costs_and_capture_links.sql` adds validated Research link snapshots and replaces the single Stay price anchor with deterministic nightly allocation. The current Apply wrapper keeps canonical mutation and durable history atomic, stores each nightly price in the Revert after-snapshot, and remains executable only by authenticated owners. Contextual **Save idea** snapshots all canonical booking fields available in the selected Plan group—Places, dates/times, journey segments, notes, source URL, and links—rather than copying only its title. Plan/Ideas query snapshots stay in memory for 30 minutes, render immediately while fresh, and revalidate in the background after transitions and Apply/Revert.

`20260811185219_expire_research_apply_after_plan_change.sql` makes Applied a one-time derived snapshot rather than a permanent label: changing or deleting an Apply-owned Flight, Stay, Train, or Rental row, or removing any Day in its complete booking span, makes that application non-current without deleting its durable history or Research source. `20260811190356_ensure_stay_apply_restores_checkout_day.sql` includes a Stay's checkout Day in the same atomic Apply/change set, so deleting that boundary expires Applied and a later Apply restores it safely. Plan mutations invalidate the in-memory Ideas query immediately, allowing the reusable source to show **Apply to Plan** again without a full navigation reload.
...
- `20260811093500_secure_schedule_triggers_and_research_indexes.sql`
...
7. Price Comparison / Research — **Plan | Ideas & Options**
   ...
   - ✅ Phase 6B-D/P0: selection, Plan Cost, currency-safe comparison/conversion, and OptionImpact — DONE
   - ✅ Phase 6B-E/P0: Review, Flight/Stay/Train/Rental Apply, automatic Plan schedule reconciliation, durable history, and conflict-safe Revert — DONE
   - ✅ Phase 6B P0.5: Trip App Bar, Context Bar, and progressive Matrix actions — DONE
8. Future work is deliberately unsequenced: provider launch, structural/date-shift planning flows, extraction/attachments, offline conflict handling, deployment, and operational polish.

Each phase is independently scoped. Deferred export and Travel Book concepts are not the current next sequence.
