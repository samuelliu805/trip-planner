  ...
  return (
    <form action={action}>
      ...
      <div className="space-y-2 sm:col-span-2">
        ...
        <Input id="title" maxLength={120} name="title" onChange={(event) => setTitle(event.target.value)} placeholder="e.g. Q4 Marketing Summit" required value={title} />
      ...
      <div className="space-y-2">
        ...
        <Select onValueChange={setCurrency} value={currency}>
          <SelectTrigger id="currency"><SelectValue aria-label={currency}>{currency}</SelectValue></SelectTrigger>
      ...
      <DialogFooter>
        <DialogClose asChild><Button disabled={pending} type="button" variant="ghost">Cancel</Button></DialogClose>
        <Button disabled={pending} type="submit">
          {pending ? <><LoaderCircle aria-hidden="true" className="size-4 animate-spin" /> Creating trip…</> : "Create Trip"}
        </Button>
      </DialogFooter>
    </form>
  );
