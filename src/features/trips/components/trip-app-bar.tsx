...
import { TripBarActionsProvider } from "./trip-bar-actions";
      ...
      {sections.map((section) => {
        return (
          <Link
            ...
            className={`flex min-h-10 min-w-0 items-center justify-center gap-2 rounded-lg px-3 text-sm font-semibold transition-[background-color,color,box-shadow] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 ${
              section.id === active
...
// The trip title, the Plan switcher, Share, and Trip settings all live in one identity
// control on the left (`variantControls`); account actions stay on /trips. That keeps this
// bar to a single row of breathing room with no competing overflow menus.
  ...
  return (
    <header className="trip-app-bar z-[70] shrink-0 border-b bg-background/95 backdrop-blur">
      <div className="trip-app-bar-inner flex h-14 min-w-0 items-center gap-2 min-[960px]:grid min-[960px]:grid-cols-[minmax(0,1fr)_auto_minmax(0,1fr)] min-[960px]:gap-4">
        <div className="trip-app-bar-leading flex min-w-0 flex-1 items-center gap-1 min-[960px]:flex-none">
          <Button asChild className="size-11 shrink-0 p-0" variant="ghost">
            <Link aria-label="Back to Trips" href="/trips">
              <ArrowLeft aria-hidden="true" className="size-4" />
            </Link>
          </Button>
          <TripBarActionsProvider value={barActions}>{variantControls}</TripBarActionsProvider>
        </div>
        ...
        <div className="ml-auto flex shrink-0 items-center justify-end gap-1.5 min-[960px]:ml-0">
