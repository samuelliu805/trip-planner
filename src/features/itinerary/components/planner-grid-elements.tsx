...
export function DayActions({
  ...
  return (
    <>
      <div
        className={`mt-2 grid overflow-hidden rounded-md border bg-background shadow-sm ${isOnlyDay ? "grid-cols-1" : "grid-cols-2"}`}
      >
        <Tooltip>
          <TooltipTrigger asChild>
            <button
              aria-label={t("Insert day below Day {day}", { day: day.day_number })}
              className={`${buttonClass} ${isOnlyDay ? "gap-1 px-1 text-xs font-medium text-primary" : ""}`}
              disabled={pending}
              onClick={(event) => {
