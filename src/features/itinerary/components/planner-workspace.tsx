...
import { ArrowLeft, CalendarDays, Check, ChevronDown, ClipboardPaste, Copy, MapPinned, MoreHorizontal, Plus, Route, Settings2 } from "lucide-react";
...
function AddItemPopover({ category, day, onComplex }: { category: (typeof categories)[number]; day: PlannerDay; onComplex: () => void; tripId: string; variantId: string }) {
  return <button aria-label={`Add ${category.label.toLowerCase()} on day ${day.day_number}`} className="absolute bottom-1 right-1 z-10 flex size-7 items-center justify-center rounded border bg-background text-muted-foreground opacity-0 shadow-sm transition-opacity hover:bg-muted hover:text-foreground focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring" data-add-item onClick={(event) => { event.stopPropagation(); onComplex(); }} type="button"><Plus className="size-3.5" /></button>;
}
...
export function PlannerWorkspace({ deleteError, initialWorkspace, settings, trip }: { deleteError: boolean; initialWorkspace: PlannerWorkspaceData; settings: React.ReactNode; trip: Tables<"trips"> }) {
  ...
  return <div className="flex h-full min-h-0 flex-col bg-background" onCopy={(event) => { const payload = clipboardPayload(); if (payload) { event.preventDefault(); event.clipboardData.setData("text/plain", encodePlannerClipboard(payload)); setInternalClipboard(payload); setInteractionError(undefined); } }} onPaste={(event) => { const payload = parsePlannerClipboard(event.clipboardData.getData("text/plain")) ?? internalClipboard; if (!payload) { setInteractionError("Unsupported clipboard data. Copy cells from this planner before pasting."); return; } event.preventDefault(); void pastePayload(payload); }}>
    ...
    <div className="grid min-h-0 flex-1 overflow-hidden" ref={containerRef} style={{ gridTemplateColumns: gridTemplate }}><section aria-label="Editable trip planning matrix" className="min-w-0 overflow-auto bg-background"><div className="min-w-max" data-fill-dragging={isFillDragging || undefined} role="grid" aria-label={`${trip.title} itinerary`} aria-multiselectable="true" aria-rowcount={workspace.days.length + 1} aria-colcount={9} onDoubleClick={openEditorFromDoubleClick} onPointerDown={startRangeSelection}><div className="sticky top-0 z-30 flex h-9 border-b bg-muted/95 text-[11px] font-semibold text-muted-foreground" role="row"><div className="sticky left-0 z-40 flex w-24 shrink-0 items-center border-r bg-muted px-2" role="columnheader">Date</div><div className="sticky left-24 z-40 flex w-16 shrink-0 items-center border-r bg-muted px-2" role="columnheader">Day</div>{categories.map((category) => <div className={`${category.width} flex shrink-0 items-center border-r px-2`} key={category.id} role="columnheader">{category.label}</div>)}</div>
      ...
      <div aria-label="Resize matrix and map" aria-orientation="vertical" aria-valuemax={68} aria-valuemin={45} aria-valuenow={Math.round(split)} className="relative z-40 cursor-col-resize bg-border hover:bg-primary focus-visible:bg-primary focus-visible:outline-none" onKeyDown={(event) => { if (event.key === "ArrowLeft") setSplit((value) => Math.max(45, value - 2)); if (event.key === "ArrowRight") setSplit((value) => Math.min(68, value + 2)); }} onPointerDown={startResize} role="separator" tabIndex={0} /><MapShell /></div>
