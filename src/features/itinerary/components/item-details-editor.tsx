...
const typeLabels = Object.fromEntries(
  itineraryItemTypes.map((type) => [type, type.replace("_", " ")]),
) as Record<ItineraryItem["type"], string>;
    ...
    <div className="grid gap-4 border-t bg-muted/20 p-4 md:grid-cols-2">
      ...
      <div className="space-y-1.5">
        ...
        <select
          ...
          id={`type-${item.id}`}
          onChange={(event) => setType(event.target.value as ItineraryItem["type"])}
          value={type}
        >
          {itineraryItemTypes.map((value) => (
        ...
        <>
          <div className="space-y-1.5">
            ...
            <select
              className="h-10 w-full rounded-md border bg-background px-3 text-sm"
              id={`action-${item.id}`}
              onChange={(event) => setCarAction(event.target.value as CarRentalDetails["action"])}
              value={carAction}
            >
