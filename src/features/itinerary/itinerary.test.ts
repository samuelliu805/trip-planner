...
test("spreadsheet UI uses tap-to-place Activity ordering plus rollback hooks", async () => {
  ...
  assert.match(workspace, /mt-auto flex h-9 w-full/);
  ...
  assert.match(
    styles,
    /planner-editor-sheet \{[\s\S]*inset: var\(--app-viewport-top, 0px\) 0 auto 0 !important;[\s\S]*height: var\(--app-viewport-height, 100dvh\)/,
  );
...
test("mobile and tablet workspaces contain scrolling and keep frozen Matrix layers", async () => {
  ...
  const viewportContainment = await readFile(
    new URL("../../lib/use-app-viewport.ts", import.meta.url),
    "utf8",
  );
  const appBar = await readFile(
    new URL("../trips/components/trip-app-bar.tsx", import.meta.url),
    "utf8",
  );
  ...
  assert.match(styles, /planner-map-sheet[\s\S]*height: calc\(\s*var\(--app-viewport-height/);
  ...
  assert.match(
    styles,
    /\.trip-planner-page \{[\s\S]*position: fixed;[\s\S]*inset: var\(--app-viewport-top, 0px\) 0 auto 0;[\s\S]*height: var\(--app-viewport-height, 100dvh\)/,
  );
  assert.match(
    styles,
  ...
  assert.match(tripShellRule, /height: var\(--app-viewport-height, 100dvh\)/);
  ...
  // iOS reveals a focused field by offsetting the visual viewport, which a fixed shell
  // cannot scroll back. The shell follows that geometry instead, and `resizes-content`
  // keeps the layout viewport itself in step on browsers that support it.
  assert.match(appBar, /useAppViewport\(\)/);
  ...
  assert.match(viewportContainment, /--app-viewport-height/);
  assert.match(viewportContainment, /--app-viewport-top/);
  // The override only applies while the visual viewport is genuinely obscured, so a stale
  // or missed measurement can never strand the shell short of the viewport bottom.
  assert.match(viewportContainment, /const obscured =/);
  assert.match(viewportContainment, /publish\(VIEWPORT_HEIGHT, obscured \? height : null\)/);
  // Measurement must not depend on requestAnimationFrame, which a throttled tab may skip.
  assert.doesNotMatch(viewportContainment, /requestAnimationFrame/);
