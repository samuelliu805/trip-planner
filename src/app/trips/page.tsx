...
import { resolveTripStatusFilter } from "@/features/trips/status";
import type { TripListEntry } from "@/features/trips/types";

export const metadata = { title: "Trips" };

const emptyCopy: Record<string, { body: string; title: string }> = {
  all: {
...
export default async function TripsPage({
  ...
  const [{ status }, siteUrl] = await Promise.all([searchParams, getRequestSiteUrl()]);
          ...
          {!error && trips?.length === 0 ? (
            <Card className="border-dashed">
              <CardContent className="flex min-h-52 flex-col items-center justify-center text-center">
                ...
                <p className="mt-1 max-w-xs text-sm text-muted-foreground">{empty.body}</p>
              </CardContent>
            </Card>
          ) : null}
          <div className="grid gap-4 md:grid-cols-2 lg:gap-6">
            {trips?.map((trip) => (
              <TripCard key={trip.id} siteUrl={siteUrl} trip={trip as TripListEntry} />
            ))}
          </div>
