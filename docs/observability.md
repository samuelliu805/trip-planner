...
Confirm the call uses `captureException` or `captureExceptionImmediate`, not generic `capture`. Inspect `$exception_list` for a bounded type, value, stacktrace, and `mechanism`. Every JavaScript frame must retain the required `function` field; the sanitizer uses `?` when no safe name exists. Check the `error_tracking_exception_processing_errors` ingestion warning before changing grouping logic, then check grouping and suppression rules and confirm the exception was not transformed by a generic property sanitizer. In `posthog-node@5.51.3`, `_originatedFromCaptureException` is not available inside `before_send`; requiring it drops the occurrence before any HTTP request. For Vercel smoke calls, both immediate capture and `flush()` must complete before the `202` response.
