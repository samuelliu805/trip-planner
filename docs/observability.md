...
Every event below requires a UUID `operation_id`; it becomes the stable `$insert_id` together with the event name. A retry caused by a new explicit user action gets a new operation ID. Client/server delivery of the same outcome reuses the operation and event name, so ingestion deduplicates it. `feature_area` is derived from the event name inside the shared boundary rather than accepted from feature code.
...
| Event names                                                                                              | Question and client intent owner                                                                                  | Authoritative outcome owner                                      | Allowed semantic properties                                            | Identity and dedup                                           | Safe Preview / failure evidence                                                  |
| -------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------- | ---------------------------------------------------------------------- | ------------------------------------------------------------ | -------------------------------------------------------------------------------- |
...
| `share_export_started`, `share_exported`, `share_export_failed`                                          | Does Timeline image export finish? Long-image generate owner.                                                     | Finalize/fail RPC; duplicate server reports share one insert ID. | `share_artifact=image`, `export_mode`, `surface`, failure `error_code` | Authenticated; matching operation.                           | Export a disposable image / render, upload, and finalize failure tests.          |
...
| `attachment_upload_started`, `attachment_uploaded`, `attachment_upload_failed`                           | Do explicit per-file uploads finish? Attachment queue/retry owner.                                                | Duplicate reservation or verified finalize/fail boundary.        | `attachment_target`, `surface`, failure `error_code`                   | Authenticated; matching operation; retry gets a new one.     | Upload disposable safe files / prepare, storage, and verification failure tests. |
...
Attachment events never include filename, MIME string, storage key, hash, URL, byte size, content, count, attachment ID, item ID, or research ID. Public-share events are rejected unless `actor_type=anonymous`; the token is normalized out of the route and cannot enter an allowlist.
...
20. Publish a disposable Share Page, update one setting, copy and open its link, then open that link in a separate signed-out browser profile. Change among Overview, Table, and Timeline. The public events must remain anonymous and contain no public token. Keep the page live until the next step.
