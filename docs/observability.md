...
Phase 1 establishes a privacy-safe, provider-neutral telemetry boundary for the web application. Phase 2A adds semantic Authentication, Trip lifecycle, and core Planner telemetry through that boundary. The Global launch checkpoint extends the same contract to Research, Routes, Variants, Sharing, and Attachments. Application and feature code call the typed APIs in `src/lib/telemetry`; only adapter files import PostHog or OpenTelemetry packages.
...
- `events.ts`, `events-product.ts`, and `feature-area.ts` form the typed event, property, feature-area, log-name, provider, outcome, and safe-error-code registry.
...
- the bounded properties enumerated in the product event catalogs below;
...
The HMAC is computed only on the server after `supabase.auth.getUser()` returns a real user. Only the pseudonymous ID and bounded person properties reach the client bridge. Repeated renders do not repeat `identify`; a different authenticated ID resets the previous PostHog identity first. A successful server sign-out captures `signed_out` before redirecting, and entering the anonymous login boundary resets persisted identity exactly once for that authenticated-to-anonymous transition. Login and signup use that reset boundary. Public-share routes instead initialize a separate memory-only PostHog instance behind the same provider-neutral analytics API. They never initialize, identify, or reset the persisted owner instance, so a same-origin public tab remains anonymous without changing the authenticated owner tab.
...
## Global advanced product telemetry

Every event below requires a UUID `operation_id`; it becomes a stable correlation `$insert_id` together with the event name. A retry caused by a new explicit user action gets a new operation ID. PostHog does not deduplicate custom events from this field, so application boundaries must emit each logical outcome once. `feature_area` is derived from the event name inside the shared boundary rather than accepted from feature code.

The additional bounded values are:

- `feature_area`: `ideas`, `research`, `routes`, `variants`, `sharing`, or `attachments`.
- `ideas_category`: `flight`, `rental`, `stay`, or `train`.
- `route_view`: `day` or `overview`; `route_mode` is one explicit saved route mode plus `mixed` or `unset`.
- `comparison_scope`: `day`, `trip`, or `summary`; `selection_state`: `selected` or `deselected`.
- `variant_action`: `blank` or `duplicate`.
- `share_artifact`: `page` or `image`; `export_mode`: `new` or `replace`; `public_view`: `overview`, `table`, or `timeline`.
- `attachment_target`: `itinerary` or `research`.
- Advanced surfaces are `ideas_options`, `research_editor`, `route_panel`, `variant_controls`, `variant_comparison`, `share_dialog`, `public_share`, `attachment_editor`, and `export_panel`.

Common context and prohibited-field rules are omitted from the table. “Server test” means focused automated success/failure ownership plus the shared fail-safe outcome reporter; unsafe Preview failures are never triggered against real data.

| Event names                                                                                              | Question and client intent owner                                                                                  | Authoritative outcome owner                                                                                                                                  | Allowed semantic properties                                            | Identity and correlation                                           | Safe Preview / failure evidence                                                                     |
| -------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------ | ---------------------------------------------------------------------- | ------------------------------------------------------------------ | --------------------------------------------------------------------------------------------------- |
| `ideas_viewed`, `ideas_category_changed`                                                                 | Which Ideas category is exposed or deliberately selected? `CompareWorkspace` mount guard and category navigation. | Browser semantic change only.                                                                                                                                | `ideas_category`, `surface`                                            | Authenticated; event + operation.                                  | Open and change a category / rerender-dedup test.                                                   |
| `research_create_started`, `research_created`, `research_create_failed`                                  | Do users start and durably save an option? Research form or planner capture action.                               | Research insert and RPC result.                                                                                                                              | `ideas_category`, `surface`, failure `error_code`                      | Authenticated; matching operation.                                 | Create a disposable option / server failure test.                                                   |
| `research_updated`, `research_update_failed`, `research_deleted`, `research_delete_failed`               | Are saved options maintained successfully? No decorative intent event.                                            | Research update/delete result.                                                                                                                               | `ideas_category`, `surface`, failure `error_code`                      | Authenticated; event + operation.                                  | Edit and delete a disposable option / server failure tests.                                         |
| `research_apply_started`, `research_applied`, `research_apply_failed`                                    | Is a researched option intentionally applied to a Plan? Apply action owner.                                       | Apply RPC result, before non-authoritative read-back.                                                                                                        | `ideas_category`, `surface`, failure `error_code`                      | Authenticated; matching operation.                                 | Apply a disposable option / RPC failure test.                                                       |
| `research_revert_started`, `research_reverted`, `research_revert_failed`                                 | Is an applied decision intentionally reverted? Revert action owner.                                               | Revert RPC status, including bounded conflict failure.                                                                                                       | `ideas_category`, `surface`, failure `error_code`                      | Authenticated; matching operation.                                 | Revert the disposable application / conflict test.                                                  |
| `route_calculation_started`, `route_calculated`, `route_calculation_failed`                              | Do explicit Day or Overview calculations finish? Route editor calculate action.                                   | Saved-plan workflow and route provider/persistence result.                                                                                                   | `route_view`, `route_mode`, `surface`, failure `error_code`            | Authenticated; matching operation across save and calculate.       | Calculate disposable routes / configuration and provider failure tests.                             |
| `route_mode_changed`, `route_view_changed`                                                               | Which bounded route presentation is deliberately chosen? Day/Overview mode controls and planner map mode owner.   | Browser semantic change only.                                                                                                                                | `route_view`, `route_mode`, `surface`                                  | Authenticated; event + operation.                                  | Change a route mode/view / invalid-enum test.                                                       |
| `variant_created`, `variant_create_failed`, `variant_updated`, `variant_update_failed`                   | Are Plans created, duplicated, and renamed successfully? Operation begins in the editor.                          | Variant lifecycle RPC result.                                                                                                                                | Create has `variant_action`; `surface`; failure `error_code`           | Authenticated; event + operation.                                  | Create, duplicate, edit disposable Plans / RPC failure tests.                                       |
| `variant_deleted`, `variant_delete_failed`, `variant_primary_set`, `variant_primary_set_failed`          | Do management decisions persist? Operation begins in Manage Plans.                                                | Variant lifecycle RPC result.                                                                                                                                | `surface`, failure `error_code`                                        | Authenticated; event + operation.                                  | Set primary and delete a disposable Plan / RPC failure tests.                                       |
| `variant_switched`                                                                                       | Do users deliberately switch active Plans? Route variant controls.                                                | Browser navigation owner.                                                                                                                                    | `surface`                                                              | Authenticated; event + operation.                                  | Switch Plans / same-variant no-op test.                                                             |
| `variant_comparison_viewed`, `variant_comparison_selection_changed`, `variant_comparison_summary_viewed` | Is comparison used and which bounded scope/visibility decision is selected? Planner comparison owners.            | Browser semantic change only.                                                                                                                                | `comparison_scope`, optional `selection_state`, `surface`              | Authenticated; event + operation.                                  | Open comparison, toggle one Plan, open summary / invalid-enum test.                                 |
| `share_publish_started`, `share_published`, `share_publish_failed`                                       | Do owners intentionally publish and succeed? Share dialog publish action.                                         | Share-page create RPC and validated result.                                                                                                                  | `share_artifact=page`, `surface`, failure `error_code`                 | Authenticated; matching operation.                                 | Publish a disposable page / RPC failure test.                                                       |
| `share_settings_updated`, `share_settings_update_failed`, `share_revoked`, `share_revoke_failed`         | Are published settings and revocations durable? Operation begins in Share dialog/export panel.                    | Share-page or image RPC/storage result.                                                                                                                      | `share_artifact`, `surface`, failure `error_code`                      | Authenticated; event + operation.                                  | Update and revoke disposable artifacts / server failure tests.                                      |
| `share_link_copied`, `share_link_opened`                                                                 | Are useful page-link actions taken? Share status and open-page controls.                                          | Browser interaction only.                                                                                                                                    | `share_artifact=page`, `surface`                                       | Authenticated or anonymous by surface; event + operation.          | Copy/open a disposable link / allowlist test.                                                       |
| `share_export_started`, `share_exported`, `share_export_failed`                                          | Does Timeline image export finish? Authenticated prepare owns start after Share Page ownership validation.        | Prepare owns post-auth preparation failures, finalize owns success, and fail-version owns every post-prepare failure; each operation has one terminal owner. | `share_artifact=image`, `export_mode`, `surface`, failure `error_code` | Authenticated server identity; matching operation and owner route. | Export a disposable image / render, upload, and finalize failure tests.                             |
| `public_share_viewed`, `public_share_view_changed`                                                       | Do anonymous viewers load and change the three public views? Public template controller mount guard and switcher. | Browser semantic change only.                                                                                                                                | `public_view`, `surface=public_share`                                  | Anonymous; event + operation; no token.                            | Open the disposable link and change view / anonymity and token-stripping test.                      |
| `attachment_upload_started`, `attachment_uploaded`, `attachment_upload_failed`                           | Do explicit per-file uploads finish? Attachment queue/retry owner.                                                | Prepare, verified finalize, or lifecycle fail boundary acknowledges the one outcome; the component reports only when no acknowledgement returns.             | `attachment_target`, `surface`, failure `error_code`                   | Authenticated; matching operation; retry gets a new one.           | Upload disposable safe files / prepare, storage, verification, cleanup-request, and fallback tests. |
| `attachment_opened`, `attachment_deleted`, `attachment_delete_failed`                                    | Are private previews opened and attachments removed? Attachment card and delete owner.                            | Browser open; detach RPC for delete.                                                                                                                         | `attachment_target`, `surface`, failure `error_code`                   | Authenticated; event + operation.                                  | Preview and delete disposable files / detach failure test.                                          |

Attachment events never include filename, MIME string, storage key, hash, URL, byte size, content, count, attachment ID, item ID, or research ID. A bounded `failureReported` response acknowledges lower-boundary ownership and suppresses the component fallback; a failed or unacknowledged lifecycle request retains that fallback, and `AbortError` reports nothing. Public-share events are rejected unless `actor_type=anonymous`; the token is normalized out of the route and cannot enter an allowlist.
...
16. Inspect raw Phase 2A events for the entire prohibited-field list, confirm queries are absent from routes, confirm authenticated distinct IDs match `^tpv1_[0-9a-f]{64}$`, and confirm each application-owned logical outcome appears once. Use `$insert_id` only to correlate attempts; do not infer PostHog deduplication from it. Repeat Production verification separately before claiming Production acceptance.
17. In one disposable Trip, open Ideas & Options, change category once, create and edit a disposable option, Apply it, Revert it, and delete it. Preserve the operation order so intent/outcome correlation can be checked.
18. Configure and calculate one disposable Day route; change one route mode; switch to Overview and calculate it if the disposable Trip has two city stages. A retry must be a new explicit click and therefore a new operation ID.
19. Create one blank Plan and one duplicate Plan, switch between them, rename one, set one Primary, open whole-trip and Day comparison, toggle one visible Plan, open Decision summary, and delete the disposable non-primary Plan.
20. Publish a disposable Share Page, update one setting, copy its link, then open that link in a second same-origin tab while the owner tab remains authenticated. Change among Overview, Table, and Timeline, then trigger another owner browser event. The public events must remain anonymous and contain no public token, and the owner event must retain its existing `tpv1_` identity without a reload. Keep the page live until the next step.
21. Generate a disposable Timeline image export, then revoke it. Upload one small supported attachment to an itinerary item and one to a research option, preview both, and delete both. Use only synthetic files with no real names, bookings, or personal content.
22. Revoke the disposable Share Page and remove all disposable Trip, Research, Variant, export, and attachment data. Report completion to the engineer; do not inspect PostHog or send any credential.
23. The engineer must query PostHog directly for `environment=preview`, verify event order and operation correlation, singular delivery, authenticated versus anonymous identity, raw property privacy, absence of Production contamination, and Vercel runtime errors. Unsafe failure paths are accepted only from the focused automated tests.
...
Future product work must not reuse a nearby event with misleading semantics or bypass the typed provider boundary. The separate CN adapter phase must preserve this catalog while disabling Global PostHog whenever `region=cn` until a region-appropriate adapter is implemented and accepted.
