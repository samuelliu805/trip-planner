...
- Service-role and cleanup RPC grants are intentionally absent. Current CloudBase RPC gateway
  behavior means Phase 4 cannot rely on `GRANT EXECUTE`; each privileged operation needs an
  authenticated server boundary plus internal role/ownership checks.
